...
import { reverseGeocodeCoordinatesWithGoogleMaps } from "@/lib/location";
...
const formatDetectedLocation = (
  data: { address?: Record<string, string>; name?: string; display_name?: string },
  fallback: string
) => {
...
const ProfileSetup = () => {
        ...
        try {
          ...
          if (existingProfile) {
            ...
            setForm({
              ...
              coordinates: existingProfile.coordinates || undefined,
            });
            setIsEditing(true);
          }
        } catch (error) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          try {
            address = await reverseGeocodeCoordinatesWithGoogleMaps(
              { lat: latitude, lng: longitude },
              form.language || language || "en"
            );
          } catch {
            ...
            if (response.ok) {
              ...
              address = formatDetectedLocation(data, address);
            }
          }
  ...
  const handleLocationChange = (e: ChangeEvent<HTMLInputElement>) => {
    setForm(prev => ({ ...prev, location: e.target.value, coordinates: undefined }));
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Always navigate after localStorage save so client-side blockers
      // cannot prevent the user from continuing through onboarding.
